...
export default class TransaksesController {
  ...
  public async getTransaksiTotalByMonth({}: HttpContextContract) {

    // gettotalsumbythismonth from dtl_transaksi
    ...
    const total = await Database.from("dtl_transaksis")

      .innerJoin("transaksis", "dtl_transaksis.id_transaksi", "transaksis.id")

      .where("transaksis.tgl_transaksi", "like", `%${year}-${month}%`)

      .sum("dtl_transaksis.total as total");


    let datas = {

      message: "Pendaapatan bulanan",
  ...
  public async getTransaksiTotalByDay({}: HttpContextContract) {

    // getTotalSumByThisDay From dtl_transaksi
    ...
    const total = await Database.from("dtl_transaksis")
      ...
      .where("transaksis.tgl_transaksi", "like", `%${year}-${month}-${day}%`)
